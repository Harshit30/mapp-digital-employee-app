const initialState = {
    employee: [
        {
            "customerID": 1,
            "name": {
                "first": "Peter",
                "last": "Smith"
            },
            "birthday": "1996-10-12",
            "gender": "m",
            "lastContact": "2017-06-01T23:28:56.782Z",
            "customerLifetimeValue": 191.12
        },
        {
            "customerID": 2,
            "name": {
                "first": "Anna",
                "last": "Hopp"
            },
            "birthday": "1987-05-03",
            "gender": "w",
            "lastContact": "2017-07-08T13:18:56.888Z",
            "customerLifetimeValue": 50.99
        },
        {
            "customerID": 3,
            "name": {
                "first": "Christian",
                "last": "Cox"
            },
            "birthday": "1991-02-21",
            "gender": "m",
            "lastContact": "2017-08-01T11:57:47.142Z",
            "customerLifetimeValue": 0
        },
        {
            "customerID": 4,
            "name": {
                "first": "Roxy",
                "last": "Fox"
            },
            "birthday": "1979-06-30",
            "gender": "w",
            "lastContact": "2017-01-29T21:08:50.700Z",
            "customerLifetimeValue": 213.12
        },
        {
            "customerID": 5,
            "name": {
                "first": "Eric",
                "last": "Adam"
            },
            "birthday": "1969-11-21",
            "gender": "m",
            "lastContact": "2017-03-18T12:20:06.702Z",
            "customerLifetimeValue": 1019.91
        }
    ],
    loading: false
}

const reducer = (state=initialState, action) => {
    switch(action.type) {
        case 'INITIATE_FETCH_EMPLOYEE': {
            return {
                ...state,
                loading: true
            }
        }
        
        case 'FETCH_EMPLOYEE_SUCCESS': {
            return {
                ...state,
                loading: false
            }
        }

        case 'ADD_EMPLOYEE' : {
            return {
                ...state,
                employee: state.employee.concat(action.empData)
            }
        }

        case 'UPDATE_EMPLOYEE': { 
            return {
                ...state,
                employee: state.employee.map( emp => {
                    if(emp.customerID === action.empData.customerID) {
                        return action.empData
                    }
                    return emp
                })
            }
        }

        case 'DELETE_EMPLOYEE': {
            return {
                ...state,
                employee: state.employee.filter(emp => emp.customerID !== action.customerId)

            }
        }

        default: return state
        }

    }

export default reducer